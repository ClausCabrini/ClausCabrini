...
{};this.tasks=[];this.onMessageBound=this.onMessage.bind(this);a.autoconnect&&this.connect();void 0===window.Asc&&(window.Asc={});void 0===window.Asc.scope&&(window.Asc.scope={})}e.prototype.onMessage=function(a){if("string"==typeof a.data){var b={};try{b=JSON.parse(a.data)}catch(c){b={}}if("onExternalPluginMessageCallback"===b.type&&(b=b.data,this.guid===b.guid))switch(b.type){case "onMethodReturn":0<this.callbacks.length&&(a=this.callbacks.shift())&&a(b.methodReturnData);0<this.tasks.length&&this.sendMessage(this.tasks.shift());
break;case "onCommandCallback":0<this.callbacks.length&&(a=this.callbacks.shift())&&a(b.commandReturnData);0<this.tasks.length&&this.sendMessage(this.tasks.shift());break;case "onEvent":b.eventName&&this.events[b.eventName]&&this.events[b.eventName].call(this,b.eventData)}}};e.prototype.sendMessage=function(a){var b={frameEditorId:"iframeEditor",type:"onExternalPluginMessage",subType:"connector"};b.data=a;b.data.guid=this.guid;a=this.frame;"string"===typeof a&&(a=document.getElementById(this.frame));
a&&a.contentWindow.postMessage(JSON.stringify(b),"*")};e.prototype.connect=function(){this.isConnected?console.log("This connector is already connected"):(window.addEventListener?window.addEventListener("message",this.onMessageBound,!1):window.attachEvent&&window.attachEvent("onmessage",this.onMessageBound),this.isConnected=!0,this.sendMessage({type:"register"}))};e.prototype.disconnect=function(){this.isConnected?(window.removeEventListener?window.removeEventListener("message",this.onMessageBound,
!1):window.detachEvent&&window.detachEvent("onmessage",this.onMessageBound),this.isConnected=!1,this.sendMessage({type:"unregister"}),this.sendMessage({type:"unregister"})):console.log("This connector is already disconnected")};e.prototype.callCommand=function(a,b,c){this.isConnected?(this.callbacks.push(b),a="var Asc = {}; Asc.scope = "+JSON.stringify(window.Asc.scope||{})+"; var scope = Asc.scope; ("+a.toString()+")();",c={type:"command",recalculate:!0===c?!1:!0,data:a},1!==this.callbacks.length?
this.tasks.push(c):this.sendMessage(c)):console.log("Connector is not connected with editor")};e.prototype.executeMethod=function(a,b,c){this.isConnected?(this.callbacks.push(c),a={type:"method",methodName:a,data:b},1!==this.callbacks.length?this.tasks.push(a):this.sendMessage(a)):console.log("Connector is not connected with editor")};e.prototype.attachEvent=function(a,b){this.isConnected?(this.events[a]=b,this.sendMessage({type:"attachEvent",name:a})):console.log("Connector is not connected with editor")};
e.prototype.detachEvent=function(a){this.events[a]&&(delete this.events[a],this.isConnected?this.sendMessage({type:"detachEvent",name:a}):console.log("Connector is not connected with editor"))};e.prototype._correctContextMenuItems=function(a){for(var b={guid:this.guid,items:[]},c=function(l,f){var g={id:void 0!==f.id?f.id:n(),text:f.text,disabled:f.disabled};f.data&&(g.data=f.data);if(f.items){g.items=[];for(var m=0,p=f.items.length;m<p;m++)g.items.push(c(l,f.items[m]))}else f.onClick&&(l.contextMenuEvents[g.id]=
!0,l.attachEvent(g.id,f.onClick));return g},d=0,h=a.length;d<h;d++)b.items.push(c(this,a[d]));return b};e.prototype.addContextMenuItem=function(a){this.events.onContextMenuClick||(this.events.onContextMenuClick=function(c){var d=void 0,h=c.indexOf("_oo_sep_");-1!==h&&(d=c.substring(h+8),c=c.substring(0,h));this.events[c]&&this.events[c].call(this,d)}.bind(this));if(this.contextMenuEvents)for(var b in this.contextMenuEvents)this.contextMenuEvents.hasOwnProperty(b)&&this.detachEvent(b);this.contextMenuEvents=
{};this.executeMethod("AddContextMenuItem",[this._correctContextMenuItems(a)])};e.prototype.updateContextMenuItem=function(a){this.executeMethod("UpdateContextMenuItem",[this._correctContextMenuItems(a)])};k.Asc=k.Asc?k.Asc:{};k.Asc.EditorConnector=e})(window);
